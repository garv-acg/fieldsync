function MyShifts({user,games,da,workers,locs,isPub,getRsvp,setRsvpStatus,requests,offerShift,claimShift,getDragger}){
  const myG=user.role==="umpire"?games.filter(g=>g.status==="scheduled"&&(g.ump1===user.id||g.ump2===user.id)).sort((a,b)=>new Date(a.date)-new Date(b.date)):[];
  const myD=Object.entries(da).filter(([k,v])=>{const role=user.role==="field"?"fieldCrew":"concessions";return(v[role]||[]).includes(user.id)}).sort((a,b)=>a[0].localeCompare(b[0]));

  // Shifts this worker has already offered up
  ...
  // Open offers from same-role teammates this worker can claim
  const openOffers=requests.filter(r=>r.type==="shift_offer"&&r.workerId!==user.id&&r.status==="pending"&&r.role===user.role);
  ...
  return R("div",null,
    ...
    (user.role==="field"||user.role==="concessions")&&(myD.length===0
      ? R("div",{className:"card"},R("div",{className:"empty"},"No days assigned yet."))
      : myD.map(([k,v])=>{
          const[date,locId]=k.split("|"),loc=locs.find(l=>l.id===locId),role=user.role==="field"?"fieldCrew":"concessions",tm=(v[role]||[]).filter(id=>id!==user.id).map(id=>workers.find(w=>w.id===id)?.name||"?"),p=isPub(date),offered=isOffered(date,locId);
          const isDragger=user.role==="field"&&getDragger(date,locId)===user.id;
          return R("div",{key:k,style:{background:"#181C27",border:"1px solid "+(isDragger?"#4F8A5A":"#2E3450"),borderRadius:10,padding:"14px",marginBottom:10}},
            ...
            R("div",{style:{fontSize:12,color:"#9BA3BF",marginTop:3}},(loc?.name||"?")+" — "+rl(user.role)),
            ...
            p&&R("div",null,
              ...
              R("button",{
                ...
                style:{marginTop:8},
                disabled:offered,
                onClick:()=>offerShift(user.id,date,locId,user.role,rl(user.role)+" shift on "+date+" at "+(loc?.name||"?"))
              },offered?"⏳ Offered up":"Offer up this shift")
            )
          );
        })
