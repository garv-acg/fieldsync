function WHome({user,games,da,workers,locs,isPub,rsvp,setRsvpStatus,getRsvp}){
  ...
  const gw=date=>{
    ...
    const dl=locs.filter(loc=>{const d=da[dk(date,loc.id)]||{},role=user.role==="field"?"fieldCrew":"concessions";return(d[role]||[]).includes(user.id)});
  ...
  return R("div",null,
    R("div",{style:{background:"#1A2550",border:"1px solid #4F7EF7",borderRadius:12,padding:"16px 20px",marginBottom:18,display:"flex",alignItems:"center",gap:14}},
      ...
      R("span",{className:"badge "+rb(user.role),style:{marginLeft:"auto"}},rl(user.role))
    ),
    ...
    R("div",{className:"card"},
      ...
      R("div",null,
        ...
        sw.days.map(loc=>{
          const k=dk(sel,loc.id),d=da[k]||{},role=user.role==="field"?"fieldCrew":"concessions",tm=(d[role]||[]).filter(id=>id!==user.id).map(id=>workers.find(w=>w.id===id)?.name||"?");
          return R("div",{key:loc.id,style:{background:"#1E2333",border:"1px solid #2E3450",borderRadius:8,padding:"12px",marginBottom:10}},
            ...
            R("div",{style:{fontSize:12,color:"#9BA3BF",marginTop:3}},rl(user.role)+" assignment"),
